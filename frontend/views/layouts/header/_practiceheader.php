                    ...
                    <div class="primary-context overflow-hidden low-margin">
                        ...
                        <div class="title">
                            <div class="head colos regular" style="line-height: 22px; color: white; text-shadow: 1px 1px 3px #000">
                                <?= c($model->name) ?>
                                <span class="fs_11 muted">[<?= $model->location->fullAddress ?>]</span> 
                            </div>
                            <div class="subhead" style="line-height: 22px; color: #eee; text-shadow: 1px 1px 3px #000">
                                
                                <?= (1==1) ? '<div class="label label-success fs_11 thin"><i class="fa fa-check"></i> '.$model->engineer->name.'</div>' : null ?>
                            </div>
                        </div>
                        <div class="subaction">
                            <?= (\Yii::$app->user->can('updateOwnPractice', ['practice_engineer'=>$model])) ? Html::a(Yii::t('app', '<i class="fa fa-cogs"></i> Podesi'), ['update', 'id' => $model->engineer_id], ['class' => 'btn btn-default btn-sm shadow' ]) : null ?>         
                        </div>          
                    </div>
                </div>
